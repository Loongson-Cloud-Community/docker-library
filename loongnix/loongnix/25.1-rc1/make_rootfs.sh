...
: ${MIRROR_ADDRESS:="https://pkg.loongnix.cn/loongnix/25"}
: ${RELEASE:="loongnix-stable"}
...
TMPDIR=`mktemp -d`
# install packages
debootstrap --no-check-gpg --variant=minbase --components=main,non-free,contrib --arch=loong64 --foreign $RELEASE $TMPDIR $MIRROR_ADDRESS
